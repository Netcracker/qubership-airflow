...
def delete_secret_ignore_not_found(name, grace_period_seconds=20):
    try:
        v1_apps_api.delete_namespaced_secret(name=name, namespace=get_namespace(),
...
def get_redis_database():
    ...
    data = {'backupDisabled': dbaas_redis_backup_disabled,
            'type': 'redis',
            'dbOwner': dbaas_redis_db_owner,
            'originService': dbaas_redis_microservice_name,
            'classifier': {'namespace': get_namespace(),
                           'scope': 'service',
    ...
    response = requests.put(f'{dbaas_host}/api/v3/dbaas/{get_namespace()}/databases',
                            headers=headers, data=json.dumps(data), auth=auth, verify=api_verify)
    if response.status_code == 201 or response.status_code == 200:
        # ToDo do not print sensitive info
        # print(response.content)
        return json.loads(response.content)['connectionProperties']
    else:
        logging.error("[error_code=AIRFLOW-8300] Could not get DBAAS Redis database. Response code: %s",
...
def create_pg_secret():
    try:
        ...
        logging.exception("[error_code=AIRFLOW-1930] Error occurred while creating PG secret.")
...
def create_redis_secret():
    try:
        ...
        logging.exception("[error_code=AIRFLOW-1931] Error occurred while creating redis secret.")
