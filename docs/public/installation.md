...
Image Default Entrypoint is modified in order to avoid modifying /etc/passwd when username is not set. Instead [nss_wrapper](https://cwrap.org/nss_wrapper.html) is used.
...
* `airflowPodSecurityContext` template logic modified in order to remove `runAsUser` and `fsGroup` from default security if .Values.PAAS_PLATFORM parameter is set to "OPENSHIFT".
