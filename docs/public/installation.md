...
Image Default Entrypoint is modified in order to avoid modifying `/etc/passwd` when the username is not set. Instead, [nss_wrapper](https://cwrap.org/nss_wrapper.html) is used. Since celery workers use py-setproctitle, that can cause [issues with environment variables](https://github.com/dvarrazzo/py-setproctitle#environment-variables) and therefore nss_wrapper, additionally `LNAME` environment variable is exported in Entrypoint to provide username for python processes. Note, that alternatively, instead of setting `LNAME`, it is possible to set `SPT_NOENV` environment variable to avoid clobbering `/proc/PID/environ`. 
