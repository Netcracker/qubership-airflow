...
Delete Trino Connection
    ${resp}=  DELETE On Session  airflowsession  /api/v2/connections/trino_default
...
Delete Trino Catalog Variable
    ${resp}=  DELETE On Session  airflowsession  /api/v2/variables/trino_catalog_config
    Should Be Equal  ${resp.status_code}  ${204}
...
Run DAG To Check Trino Connection
    ...
    Skip If  ${TRINO_HOST} is None  Trino connection not configured in secrets
